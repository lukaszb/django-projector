"""
Standard django-authority decorators replacement.
"""
            ...
            if request.user.is_authenticated():
                ...
                for lookup_variable in lookup_variables:
                    if isinstance(lookup_variable, basestring):
                        ...
                        model, lookups = lookup_variable[0], lookup_variable[1:]
                        if len(lookups) % 2 != 0:
                            raise ValueError(
                                "Requires even number of lookup parameters, got %s" % (lookups,))
                        zipped = zip(lookups[::2], lookups[1::2])
                        lookup_dict = dict((lookup, kwargs.get(varname, None))
                            for lookup, varname in zipped)
                        ...
                        obj = get_object_or_404(model_class, **lookup_dict)
                        params.append(obj)
