...
urlpatterns += patterns('projector.views.project',
    ...
    url(r'^projects/(?P<project_slug>[-\w]+)/members/(?P<username>\w+)/delete/$',
    ...
    # Repository sources
    url(r'^projects/(?P<project_slug>[-\w]+)/src/diff/(?P<revision_old>[\w]*)-(?P<revision_new>[\w]*)/(?P<rel_repo_url>.*)$',
    ...
    url(r'^projects/(?P<project_slug>[-\w]+)/src/annotate/(?P<revision>[\w]*)/(?P<rel_repo_url>.*)$',
